...
int main(int argc, char **argv)
{
        ...
        Publisher chatter_publisher = n.advertise<String>("chatter", 1000);
        //Rate is a class the is used to define frequency for a loop. Here we send a message each two seconds.
        Rate loop_rate(0.5); //1 message per second

    int count = 0;
    while (ok()) // Keep spinning loop until user presses Ctrl+C
    {
        ...
        spinOnce(); // Need to call this function often to allow ROS to process incoming messages
